...
import logging
from utils import Explainer, Explanation, num_grad_batch, DEFAULT_META_CF, DEFAULT_DATA_CF
...
class CounterFactual(Explainer):
    ...
    def __init__(self,
        ...
        with tf.compat.v1.variable_scope('cf_search', reuse=tf.compat.v1.AUTO_REUSE):

            # define variables for original and candidate counterfactual instances, target labels and lambda
            self.orig = tf.compat.v1.get_variable('original', shape=shape, dtype=tf.float32)
            self.cf = tf.compat.v1.get_variable('counterfactual', shape=shape,
                                      dtype=tf.float32,
            ...
            self.target = tf.compat.v1.get_variable('target', shape=(self.batch_size, self.n_classes), dtype=tf.float32)
            ...
            self.lam = tf.compat.v1.placeholder(tf.float32, shape=(self.batch_size), name='lam')

            # define placeholders that will be assigned to relevant variables
            self.assign_orig = tf.compat.v1.placeholder(tf.float32, shape, name='assing_orig')
            self.assign_cf = tf.compat.v1.placeholder(tf.float32, shape, name='assign_cf')
            self.assign_target = tf.compat.v1.placeholder(tf.float32, shape=(self.batch_size, self.n_classes),
                                                name='assign_target')
            ...
            self.grad_ph = tf.compat.v1.placeholder(shape=shape, dtype=tf.float32, name='grad_cf')
        ...
        self.tf_init = tf.compat.v1.variables_initializer(var_list=tf.compat.v1.global_variables(scope='cf_search'))
